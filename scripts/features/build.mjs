...
const docLink = feature => {
  const doc = feature.docs[0] ?? '';
  return doc.startsWith('docs/') ? doc.slice('docs/'.length) : `../${doc}`;
};
const table = (ko) => manifest.features.map(feature => `| ${feature.id} | ${ko ? feature.title_ko : feature.title} | ${feature.status} | ${clients.map(client => `${client}: ${feature.clients[client]}`).join('<br>')} | [${ko ? '근거' : 'Evidence'}](${docLink(feature)}) |`).join('\n');
