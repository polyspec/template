...
타입 고정 생성기는 `make typed-generator`로 실행한다. 전체 compiled source graph manifest와 명시적 타입 manifest를 읽는다. 언어 backend를 실행하기 전에 공통 compiler IR이 모든 템플릿을 검증하고 include와 block 경로를 해석하며 root 변수, local, loop item, record member, 함수 호출의 타입을 결정한다. 그 뒤 manifest에서 도출한 assign과 record 선언을 가진 PHP·Go·Rust·TypeScript 소스를 만든다. nullable 필드는 각각 `?T`, `*T`, `Option<T>`, 선택 속성으로 변환한다. `make compiler-ir-check`는 canonical node와 expression의 모든 종류 및 선언되지 않은 symbol 거부를 검사한다. `make typed-generator-check`는 산출물 재현성을 확인하고 `make typed-generator-compile-check`는 생성 파일을 각 언어의 컴파일러 또는 문법 검사기에 입력한다. 임의의 JSON만으로는 정적 타입을 안전하게 추론할 수 없으므로 manifest가 필요하다.
