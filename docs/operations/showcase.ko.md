...
타입 고정 생성기는 `make typed-generator`로 실행한다. 정본 AST와 명시적 타입 manifest를 받아 manifest에서 도출한 assign과 record 선언을 가진 PHP·Go·Rust·TypeScript 소스를 만든다. nullable 필드는 각각 `?T`, `*T`, `Option<T>`, 선택 속성으로 변환한다. `make typed-generator-check`는 산출물 재현성을 확인하고 `make typed-generator-compile-check`는 생성 파일을 각 언어의 컴파일러 또는 문법 검사기에 입력한다. 임의의 JSON만으로는 정적 타입을 안전하게 추론할 수 없으므로 manifest가 필요하다.
