...
`Definition<T>.data`는 선언된 target의 고정된 `Input<T>` 구조를 가진다. 생성된 block은 `assign`, `definition.data`, `block scope` 순서로 입력을 만든 뒤 생성된 자식 template 함수를 직접 호출한다. `Definition<T>.html`은 이미 신뢰된 HTML을 반환하는 명시적인 분기이며, 컴파일된 template인 것처럼 취급하지 않는다.
