...
`make conformance-generated-ts` builds a fresh generated TypeScript program for every one of the 216 canonical cases. It compares compile diagnostics, input-binding diagnostics, runtime diagnostics and successful UTF-8 output with the same expected artifacts used by AST execution.
