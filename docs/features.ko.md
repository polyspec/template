...
| release-test-matrix | 계약·단위·compiler·적합성·mutation·소비·표시·성능으로 구성된 7계층 gate | implemented | passed | not-deployed | [릴리스 테스트](operations/testing.ko.md) |
| dependency-policy | 호환되는 최신 안정 release, 기계 검사하는 고정 예외, lock 보안 권고 거부와 지원 runtime CI 양 끝점 | implemented | passed | not-deployed | [의존성 정책](operations/dependencies.ko.md) |
