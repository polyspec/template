...
현재 generated mode 범위: AST 실행은 211개 적합성 suite를 통과한다. 제품 compiler artifact는 `AstProgram`과 같은 `Program` 계약을 구현하고 TypeScript, JavaScript, Go, Rust, PHP에서 showcase 시나리오 5개를 직접 실행한다. Generated 실행은 아직 `default` 내장 함수만 받고 전체 generated 적합성 matrix를 통과하지 않았다. 이 matrix와 제품 artifact 성능 측정이 통과할 때까지 성능과 showcase 검증은 pending이다.
