...
[English](/features). 계약은 [명세](/ko/)에 정의되어 있다. 검증과 배포는 따로 기록한다. `pending`은 통과 결과가 아니다.
