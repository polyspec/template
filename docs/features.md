...
Current generated-mode scope: AST execution passes the 211-case conformance suite. The product compiler artifacts implement the same `Program` contract as `AstProgram` and run the five showcase scenarios directly in TypeScript, JavaScript, Go, Rust and PHP. Generated execution still accepts only the `default` built-in and has not passed the complete generated conformance matrix. Performance and showcase verification remain pending until that matrix and the product-artifact measurements pass.
