...
Current generated-mode scope: product compiler artifacts implement the same `Program` contract as `AstProgram`. `make conformance-all-modes` passes all 1,688 core cells: 211 canonical cases in TypeScript, Go, Rust and PHP through both AST and generated execution. The generated runners compile and execute host-language artifacts and compare built-in calls, compile diagnostics, input diagnostics, runtime diagnostics and exact UTF-8 output without AST fallback. Performance, packaged consumption and showcase release verification remain pending.
