...
Current generated-mode scope: AST execution passes the 211-case conformance suite. Generated execution is verified only by the five showcase scenarios, and the typed compiler accepts only the `default` built-in. Generated artifacts now implement the same `Program` contract as `AstProgram`, and the callback path has been removed. The separate showcase generator still does not establish the complete compiler contract. Performance and showcase verification remain pending until one compiler produces the artifacts used by full generated conformance and measurement.
