...
템플릿은 태그가 들어 있는 텍스트다. 태그는 `{` 뒤에 기호 `= @ ? :? : / + # ?# *` 중 하나가 오거나, `{:a = 1}`처럼 `{:` 뒤에 식별자와 대입이 올 때 시작한다. 그 외의 `{`는 텍스트다.
...
템플릿은 내장 함수와 애플리케이션이 등록한 함수만 호출한다. assign한 객체는 선언된 public 필드와 메서드를 노출할 수 있고 선언된 논리 클래스 함수는 `Class::name(...)`으로 호출할 수 있다. 선언되지 않은 호스트 멤버에는 접근할 수 없다.
