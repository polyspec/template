...
export function createTarget({ program }) {
  ...
  target.set = (name, value, level) => indent(level, `$scope->locals->set(${quote(name)}, ${value});`);
...
export function emitTemplates(context) {
  ...
  return templateBodies.map(template => {
    ...
    return `function ${template.function}(Assign $assign, Definitions $definitions, ${template.input} $input, Context $context, RuntimeBindings $runtime, MapValue $rootData, Scope $scope): void {\n    $frame = new Frame(${quote(template.name)}, ${quote(typed.lines)}, $rootData);\n${[...template.inputs].map(([name]) => `    $scope->locals->set(${quote(name)}, $input->${fieldName(name)});`).join('\n')}\n${template.body}\n}`;
  }).join('\n');
}

export function emitEntry(context) {
  const { program, templateBodies } = context;
  const dispatch = templateBodies.filter(template => template.inputs.size === 0).map(template => `        case ${quote(template.name)}: ${template.function}($assign, $definitions, new ${template.input}(), $context, $runtime, $rootData, $scope); return;`).join('\n');
