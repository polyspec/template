...
const phpLiteral = value => quote(value).replaceAll('$', '\\$');
...
export function createTarget({ program }) {
  ...
  target.var = (name, _type, node) => program.dynamicRoot ? node.scope ? `$scope->lookup($frame, ${phpLiteral(name)})` : `$runtime->member($rootData, ${phpLiteral(name)})` : `$assign->${name}`;
  target.local = name => `$scope->lookup($frame, ${phpLiteral(name)})`;
  target.set = (name, value, level) => indent(level, `$scope->locals->set(${phpLiteral(name)}, ${value});`);
  target.member = (object, key, owner) => owner.kind === 'any' ? `$runtime->member(${object}, ${phpLiteral(key)})` : `${object}?->${key}`;
  target.text = (value, level, node) => indent(level, `$context->at($frame, ${phpLiteral(node.span)}); $context->write(${phpLiteral(value)});`);
  target.echo = (expression, level, node) => indent(level, `$context->at($frame, ${phpLiteral(node.span)}); $context->write($runtime->escape(${expression}, $frame, ${phpLiteral(node.expr.span)}));`);
  target.block = (node, n) => {
    if (node.target === null) return indent(n, `$definition = $definitions->${fieldName(node.id)};\nif ($definition?->html === null) throw $runtime->error($frame, ${phpLiteral(node.span)}, 'E_RUNTIME_BLOCK_UNDEFINED', ${phpLiteral(`define ${node.id} is not registered`)});\n$context->at($frame, ${phpLiteral(node.span)}); $context->write($definition->html);`);
    const fallbackValue = item => {
      if (item.root) return emitExpression(item.root, target);
      if (item.valueType.optional) return 'null';
      if (node.id !== null) return `($definition?->data !== null && $definition->data->has_${fieldName(item.name)} ? $definition->data->${fieldName(item.name)} : throw $runtime->error($frame, ${phpLiteral(node.span)}, 'E_RUNTIME_TYPE', ${phpLiteral(`generated input ${node.target}.${item.name} is missing`)}))`;
      return `throw $runtime->error($frame, ${phpLiteral(node.span)}, 'E_RUNTIME_TYPE', ${phpLiteral(`generated input ${node.target}.${item.name} is missing`)})`;
    };
    ...
    if (node.id === null) return indent(n, `$input = new ${inputName(node.target)}(${fallback});\n${scoped}\n$blockScope = new Scope();\n$context->enter(${phpLiteral(node.target)}, $frame, ${phpLiteral(node.span)});\ntry { ${call}; } finally { $context->leave(); }`);
    const field = fieldName(node.id);
    const registration = node.path === null
      ? `if ($definition === null) throw $runtime->error($frame, ${phpLiteral(node.span)}, 'E_RUNTIME_BLOCK_UNDEFINED', ${phpLiteral(`define ${node.id} is not registered`)});`
      : `if ($definition === null) { $definition = new Definition(template: ${phpLiteral(node.target)}); $definitions->${field} = $definition; } elseif ($definition->html !== null || $definition->template !== ${phpLiteral(node.target)}) throw $runtime->error($frame, ${phpLiteral(node.span)}, 'E_RUNTIME_BLOCK_REDEFINED', ${phpLiteral(`define ${node.id} is registered with a different template`)});`;
    return indent(n, `$definition = $definitions->${field};\n${registration}\nif ($definition?->html !== null) {\n    $context->at($frame, ${phpLiteral(node.span)}); $context->write($definition->html);\n} else {\n    if ($definition?->data !== null && !($definition->data instanceof ${definitionDataName(node.target)})) throw $runtime->error($frame, ${phpLiteral(node.span)}, 'E_RUNTIME_TYPE', ${phpLiteral(`generated definition ${node.id} data has an invalid type`)});\n    $input = new ${inputName(node.target)}(${fallback});\n    if ($definition?->data !== null) {\n${indent(2, defined)}\n    }\n${indent(1, scoped)}\n    $blockScope = new Scope();\n    $context->enter(${phpLiteral(node.target)}, $frame, ${phpLiteral(node.span)});\n    try { ${call}; } finally { $context->leave(); }\n}`);
  };
  ...
  target.binary = (operator, left, right, node) => {
    ...
    return `$runtime->binary(${phpLiteral(operator)}, ${left}, ${right}, $frame, ${phpLiteral(node.span)})`;
  };
  target.ternary = (test, thenValue, elseValue) => `($runtime->truthy(${test}) ? ${thenValue} : ${elseValue})`;
  target.list = items => `generated_list([${items.map(item => `['spread' => ${item.spread}, 'value' => ${item.spread ? `$runtime->listSpread(${item.value}, $frame, ${phpLiteral(item.span)})` : item.value}]`).join(', ')}])`;
  target.map = entries => `generated_map([${entries.map(item => item.spread ? `['spread' => true, 'value' => $runtime->mapSpread(${item.value}, $frame, ${phpLiteral(item.span)})]` : `['spread' => false, 'key' => ${item.value[0]}, 'value' => ${item.value[1]}]`).join(', ')}])`;
  ...
  target.forNode = (node, n) => {
    ...
    return indent(n, `$${name}_entries = $runtime->entries(${emitExpression(node.iter, target)}, $frame, ${phpLiteral(node.span)});\n$${name}_size = count($${name}_entries);\n$${name}_last_index = $${name}_size - 1;\n$${name}_had = $scope->locals->has(${phpLiteral(node.name)});\n$${name}_previous = $scope->locals->get(${phpLiteral(node.name)});\nforeach ($${name}_entries as $${name}_index => [$${name}_key, $${name}_value]) {\n    $scope->locals->set(${phpLiteral(node.name)}, $${name}_value);\n    $${name}_stack = $scope->loops[${phpLiteral(node.name)}] ?? [];\n    $${name}_stack[] = ['index_' => (float) $${name}_index, 'size_' => (float) $${name}_size, 'first_' => $${name}_index === 0, 'last_' => $${name}_index === $${name}_last_index, 'key_' => $${name}_key, 'value_' => $${name}_value];\n    $scope->loops[${phpLiteral(node.name)}] = $${name}_stack;\n    $context->iterations++;\n    $runtime->limit('iteration', $context->iterations, $frame, ${phpLiteral(node.span)});\n${emitNodes(node.body, target, n + 1)}\n    $${name}_stack = $scope->loops[${phpLiteral(node.name)}];\n    array_pop($${name}_stack);\n    if ($${name}_stack === []) unset($scope->loops[${phpLiteral(node.name)}]); else $scope->loops[${phpLiteral(node.name)}] = $${name}_stack;\n}\nif ($${name}_had) $scope->locals->set(${phpLiteral(node.name)}, $${name}_previous); else $scope->locals->remove(${phpLiteral(node.name)});${node.empty ? `\nif ($${name}_size === 0) {\n${emitNodes(node.empty, target, n + 1)}\n}` : ''}`);
  };
  target.include = (node, n) => indent(n, `$context->enter(${phpLiteral(node.target)}, $frame, ${phpLiteral(node.span)});\ntry { ${functionName(node.target)}($assign, $definitions, new ${inputName(node.target)}(${node.inputs.map(item => `${fieldName(item.name)}: ${emitExpression(item.value, target)}`).join(', ')}), $context, $runtime, $rootData, $scope); } finally { $context->leave(); }`);
  return target;
}
...
export function emitDeclarations(context) {
  ...
  const definitionsType = `final class Definition { public function __construct(public readonly ?string $template = null, public readonly ?string $html = null, public readonly mixed $data = null) {} }\nfinal class Definitions { public function __construct(${[...program.definitions].map(([name]) => `public ?Definition $${fieldName(name)} = null`).join(', ')}) {} }`;
