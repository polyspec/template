...
export function createTarget({ program }) {
  ...
  target.block = (node, n) => {
    ...
    return indent(n, `{ let definition = definitions.${fieldName(node.id)};\n${registration}\nif (definition?.html !== undefined) { context.at(frame, ${quote(node.span)}); context.output.write(definition.html); }\nelse { const input = Object.assign({ ${base.join(', ')} }, definition?.data ?? {}, { ${scoped.join(', ')} }) as ${inputName(node.target)}; context.enter(${quote(node.target)}, frame, ${quote(node.span)}); try { ${call}; } finally { context.leave(); } }\n}`);
  };
