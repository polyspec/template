...
export function createTarget({ program }) {
  ...
  target.block = (node, n) => {
    ...
    const missing = node.path === null ? `if (definition === undefined) throw runtime.error(frame, ${quote(node.span)}, 'E_RUNTIME_BLOCK_UNDEFINED', ${quote(`define ${node.id} is not registered`)});` : '';
