...
import { analyze, AstProgram, MapLoader } from '../../packages/template-ts/dist/index.mjs';
...
const objectCallTemplate = '<article><h1>{= order.label}</h1><p>{= order.status_label("ready")}</p><p>{= Order::suffix("!")}</p></article>\n';
class ShowcaseOrder {
  label = 'Order 12';
  status_label(prefix) { return `${prefix}:12`; }
}
const objectCallProgram = new AstProgram({
  loader: new MapLoader(new Map([['object-calls.tpl', objectCallTemplate]])),
  classFunctions: { 'Order::suffix': ([suffix]) => `done${suffix}` },
});
const objectCallOutput = objectCallProgram.render('object-calls.tpl', { order: new ShowcaseOrder() });
const objectCallSource = `class Order { public label = 'Order 12'; public status_label(prefix) { return prefix + ':12'; } }\nconst assign = { order: new Order() };`;
const objectCallSection = `<section class="panel"><div class="section-heading"><div><p class="eyebrow">03 · NATIVE VALUES</p><h2>Assign keeps the application instance</h2></div><p>Applications pass their own class instance through assign. The template reads a public field, calls the instance method and calls a separately registered logical class function.</p></div><div class="native-proof">${code('template', highlightTemplate(objectCallTemplate, 'object-calls.tpl'), true)}${code('application assign', objectCallSource)}<p class="output-label">Rendered output · ${escapeHtml(digest(objectCallOutput))}</p><pre>${highlightEscapedHtml(escapeHtml(objectCallOutput))}</pre></div></section>`;
...
<main>${objectCallSection}<section class="panel"><div class="section-heading"><div><p class="eyebrow">01 · EVIDENCE</p><h2>Same bytes from AST mode</h2></div><p>Source parsing occurs during the build. Runtime checks load the committed artifacts, apply assign and define, and compare UTF-8 output bytes. Generated mode must produce the same bytes before it can be enabled for a language.</p></div><div class="table-wrap"><table><thead><tr><th>Scenario</th><th>Bytes</th><th>SHA-256</th>${results.languages.map(language => `<th>${escapeHtml(language)}</th>`).join('')}</tr></thead><tbody>${results.scenarios.map(row => `<tr><td><a href="#${escapeHtml(row.id)}">${escapeHtml(row.id)}</a></td><td>${row.bytes}</td><td>${escapeHtml(row.sha256.slice(0, 12))}…</td>${results.languages.map(language => `<td class="${row.renders[language]?.status === 'pass' ? 'pass' : 'fail'}">${row.renders[language]?.status?.toUpperCase() ?? 'FAIL'}</td>`).join('')}</tr>`).join('')}</tbody></table></div></section>
