...
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1"><title>Template artifact lab</title><link rel="stylesheet" href="./styles.css"></head>
<body><header class="hero"><p class="eyebrow">POLYSPEC TEMPLATE · STATIC ARTIFACT PROOF</p><h1>Template artifact lab</h1><p class="lede">Every implementation reads a build-time AST artifact and renders the same assign and define input. The page itself is already complete HTML; it does not parse templates in the browser.</p><div class="status-grid"><div class="status-card"><span>Cross-language proof</span><strong class="pass">${escapeHtml(results.status.toUpperCase())}</strong></div><div class="status-card"><span>Artifact mode</span><strong class="pass">OFF</strong></div><div class="status-card"><span>Page validation</span><strong class="pass">HTML</strong></div></div></header>
<main><section class="panel"><div class="section-heading"><div><p class="eyebrow">01 · EVIDENCE</p><h2>Same bytes from compiled AST</h2></div><p>Source parsing occurs during the build. Runtime checks load the committed artifacts, apply assign and define, and compare UTF-8 output bytes.</p></div><div class="table-wrap"><table><thead><tr><th>Scenario</th><th>Bytes</th><th>SHA-256</th>${results.languages.map(language => `<th>${escapeHtml(language)}</th>`).join('')}</tr></thead><tbody>${results.scenarios.map(row => `<tr><td><a href="#${escapeHtml(row.id)}">${escapeHtml(row.id)}</a></td><td>${row.bytes}</td><td>${escapeHtml(row.sha256.slice(0, 12))}…</td>${results.languages.map(language => `<td class="${row.renders[language]?.status === 'pass' ? 'pass' : 'fail'}">${row.renders[language]?.status?.toUpperCase() ?? 'FAIL'}</td>`).join('')}</tr>`).join('')}</tbody></table></div></section>
