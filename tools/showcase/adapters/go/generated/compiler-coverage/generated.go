...
func render_layout_tpl(assign Assign, definitions Definitions, input Input_layout_tpl, context *render.Context, runtime *render.RuntimeBindings, rootData *value.OrderedMap, scope *render.Scope) {
    ...
    scope.Locals["values"] = generatedValue(func() []float64 { result := []float64{}; result = append(result, float64(0)); result = append(result, assign.Numbers...); return result }())
    scope.Locals["merged"] = generatedValue(func() OrderedMap[string, string] { result := NewOrderedMap[string, string](); for _, entry := range assign.Lookup.Entries() { result.Set(entry.Key, entry.Value) }; result.Set("z", "Z"); return result }())
    ...
    for row_index, entry := range entries {
        ...
        _ = row_key
        scope.Locals["row"] = generatedValue(row_value)
