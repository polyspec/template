...
func render_layout_tpl(assign Assign, definitions *Definitions, input Input_layout_tpl, context *render.Context, runtime *render.RuntimeBindings, rootData *value.OrderedMap, scope *render.Scope) {
	frame := render.NewFrame("layout.tpl", errs.LineIndex{0, 25, 46, 58, 69}, rootData)

    generatedWrite(context, "<section class=\"notice\">\n<h1>", frame, ast.Span{0, 29})
    generatedWrite(context, generatedEscape(runtime, assign.Heading, frame, ast.Span{32, 39}), frame, ast.Span{29, 40})
    generatedWrite(context, "</h1>\n", frame, ast.Span{40, 46})
    { definition := definitions.Content
    if definition == nil || definition.HTML == nil { panic(runtime.Error(frame, ast.Span{46, 57}, errs.RuntimeBlockUndefined, "define content is not registered")) }
    generatedWrite(context, *definition.HTML, frame, ast.Span{46, 57})
    }
    generatedWrite(context, "</section>\n", frame, ast.Span{58, 69})
}
...
func generatedBindDefinitions(input map[string]template.DefineInput) (*Definitions, map[string]generatedTarget, error) { _ = input; plain := map[string]any{}; targets := map[string]generatedTarget{}; for id, input := range input { _ = input; switch id {
		case "content":
			...
			if input.Template != "" { return nil, nil, fmt.Errorf("define.%s has an invalid template", id) }; plain[id] = map[string]any{"template": input.Template, "data": generatedPlain(input.Data)}; targets[id] = generatedTarget{target: ""}
		case "layout":
