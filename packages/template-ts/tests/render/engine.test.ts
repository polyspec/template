...
import { Engine, MapLoader, parse, TemplateError } from '../../src/index.js';
import { Engine as RenderEngine } from '../../src/render.js';

describe('Engine', () => {
  it('renders a template from a map loader', () => {
    const engine = new Engine({ loader: new MapLoader({ 'a.tpl': '<b>{= x}</b>' }) });
    expect(engine.render('a.tpl', { x: '<' })).toBe('<b>&lt;</b>');
  });

  it('calls registered host functions and binds their results', () => {
    const engine = new Engine({ loader: new MapLoader({ 'a.tpl': '{= twice(2)} {= wrap("a")}' }) });
    engine.register('twice', ([n]) => (n as number) * 2);
    engine.register('wrap', ([s]) => `[${s as string}]`);
    expect(engine.render('a.tpl', {})).toBe('4 [a]');
    expect(() => engine.register('upper', () => '')).toThrow();
  });

  it('reports host function failures at the call', () => {
    const engine = new Engine({ loader: new MapLoader({ 'a.tpl': 'x\n{= boom()}' }), functions: { boom: () => { throw new Error('no'); } } });
  ...
  it('applies the iteration limit', () => {
    const engine = new Engine({ loader: new MapLoader({ 'a.tpl': '{@ i = range(1, 10)}{= i}{/}' }), limits: { iterations: 5 } });
  ...
  it('renders a parsed AST through the render-only entry', () => {
    ...
    const engine = new RenderEngine({ loader: new MapLoader({ 'x.tpl': ast }) });
  ...
  it('re-parses when the loader reports a new version', () => {
    ...
    const engine = new Engine({ loader });
